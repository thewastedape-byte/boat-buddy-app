...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const isYard = tier === 'yard_addon' || tier === 'yard_standalone'
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'],
      mode: 'subscription',
      customer_email: email,
      line_items: [{ price: priceId, quantity: 1 }],
      ...(isYard ? {} : { discounts: [{ coupon: 'LAUNCH2026' }] }),
      ...
      cancel_url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://boatbuddy.thewastedape.com'}/${isYard ? 'yard' : 'upgrade'}`,
    })
